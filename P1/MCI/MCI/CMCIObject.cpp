...
bool CMCIObject::OpenFile(LPCWSTR pszFileName) {
	...
	if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_SET, MCI_SET_TIME_FORMAT, (DWORD_PTR)&t)) != 0) { 
		MCIError(); 
		return false; 
	}
...
bool CMCIObject::OpenAudioCD(LPCWSTR drive, BYTE& tracks) {
	...
	if ((m_Result = mciSendCommand(0, MCI_OPEN, MCI_OPEN_TYPE_ID | flag | MCI_WAIT | MCI_OPEN_TYPE, (DWORD_PTR)&m_op)) != 0) { 
		MCIError(); 
		return false; 
	} 
	...
	if ((m_Result=mciSendCommand(m_op.wDeviceID, MCI_STATUS, MCI_STATUS_ITEM, (DWORD_PTR)&status)) != 0) { 
		MCIError(); 
		return false; 
	} 
...
void CMCIObject::Close() {
	...
	if (m_op.wDeviceID != 0) {
		Stop(); // Just in case. 
		if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_CLOSE,MCI_WAIT,(DWORD_PTR)&gp)) != 0) 
			MCIError(); 
		m_op.wDeviceID = 0; } 
}
...
void CMCIObject::Play() {
	...
	if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_PLAY, 0, (DWORD_PTR)&play)) != 0) 
		MCIError(); 
}

void CMCIObject::Stop() {
	if (m_op.wDeviceID == 0) return; // Not open 
	if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_STOP, MCI_WAIT,0)) != 0) 
		MCIError(); 
}

void CMCIObject::Pause() {
	if (m_op.wDeviceID == 0) return; // Not open 
	if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_PAUSE, MCI_WAIT,0)) != 0) 
		MCIError(); 
}
...
bool CMCIObject::SetVideoPosition(HWND hwnd, CRect rect) {
	...
	if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_WINDOW,  MCI_ANIM_WINDOW_HWND, (DWORD_PTR)&wparms)) != 0) { 
		MCIError(); 
		return false; 
	} 
	
	where.rc = rect;   // set destination rect 
	if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_PUT, MCI_ANIM_RECT | MCI_ANIM_PUT_DESTINATION, (DWORD_PTR)&where)) != 0) { 
		MCIError(); 
		return false; 
	} 
...
bool CMCIObject::TMSFSeek(BYTE track, BYTE min, BYTE sek, BYTE frame) {
	...
	if (m_Result=mciSendCommand(m_op.wDeviceID, MCI_SEEK, MCI_TO|MCI_WAIT, (DWORD_PTR)(LPVOID)&s) != 0) { 
		MCIError(); 
		return false; 
	} 
	return true; 
...
bool CMCIObject::GetTrackLength(BYTE track, BYTE& min, BYTE& sek, BYTE& frame) {
	...
	if ((m_Result=mciSendCommand(m_op.wDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_TRACK, (DWORD_PTR)(LPMCI_STATUS_PARMS)&length)) != 0) { 
		MCIError(); 
		return false; 
	} 
...
int CMCIObject::GetMediaLength(BYTE track) {
...
bool CMCIObject::GetTMSFPosition(BYTE& track, BYTE& min, BYTE& sek, BYTE& frame) {
	...
	if ((m_Result=mciSendCommand(m_op.wDeviceID, MCI_STATUS, MCI_STATUS_ITEM, (DWORD_PTR)&pos)) != 0) 
		return false; 
