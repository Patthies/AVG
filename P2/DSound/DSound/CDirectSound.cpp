...
bool CDirectSound::Create(CWnd* wnd) {
	if (!lpds) { 
		...
		if (CoCreateInstance(CLSID_DirectSound, NULL, CLSCTX_ALL, IID_IDirectSound, (LPVOID*)&lpds) != DS_OK) { AfxMessageBox(L"DirectSound object not created."); 
		return false; 
		} 
		
		if (lpds->Initialize(0) != DS_OK) { AfxMessageBox(L"Error: Initialize DirectSound-Interface"); 
		return false; 
		}
...
LPDIRECTSOUNDBUFFER CDirectSound::CreateSoundBuffer(WORD Channels, WORD BitsPerSample, DWORD SamplesPerSec, WORD seconds) {
	...
	dsbd.lpwfxFormat = &pcmwf; // create the sound buffer now 
...
bool CDirectSound::GenerateSound(LPDIRECTSOUNDBUFFER buf, DWORD offset, DWORD length, int f) {
	...
	for (i=0, t=(short int*)lpvPtr1; i<(dwBytes1+dwBytes2);i+=4,t+=2) { 
		if (i==dwBytes1) t=(short int*)lpvPtr2; *t= *(t+1) = (short int) (sin(i/(pcmwf.nAvgBytesPerSec/(6.283185*f)))*3000); 
	} 
